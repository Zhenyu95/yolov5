...
Template_path = './Template_Station2'
...
## Load Parameters
idx1 = [(2787, 3520), (2744, 3433), (2785, 3517), (2695, 3487)]
idx2 = [(2122, 2754), (266, 874), (515, 1241), (1742, 2474)]
idx3 = [(2774, 3525), (2774, 3470), (930, 1486), (901, 1437)]
idx4 = [(400, 999), (2011, 2563), (524, 1238), (1783, 2492)]
...
def pred(img_path, model_list, image_folder=IMAGE_FOLDER):
    ...
    nms = non_max_suppression(pred, conf_thres, .6, classes=None, agnostic=False)[0]
    label=[]
    for *xyxy, conf, cls in nms:
        ###### if SW == 2, remove black area
        if SW == 2:
            ...
            xywh = xyxy2xywhn(torch.tensor(xyxy).view(1, 4), w=IMG_SIZE[0]*image.size[0]/image.size[1], h=IMG_SIZE[1]).view(-1).tolist()      
            ...
            if (xywh[0]*3024*math.sin(-theta_s[idx])*flag[idx] <  (xywh[1]*4032/kt2 - bt2)*math.sin(-theta_s[idx])*flag[idx]) and (xywh[1]*4032 < kt1*xywh[0]*3024 + bt1): 
                ...
                if SAVE_IMG:
                    ...
                    if SAVE_CONF:
                        ...
                        draw.text((xyxy[0]*kx, xyxy[1]*kx - h if outside else xyxy[1]*kx), label_txt, fill=(255,255,255), font=font)
...
def debug(image_folder=IMAGE_FOLDER):
    model_list = []
    ...
    for img_path in tqdm(os.listdir(image_folder)):
        ...
        if (img_path.endswith(".jpg")) and (not img_path.startswith('.')):
            pred(img_path, model_list, image_folder)
...
def main(image_folder=IMAGE_FOLDER):
    model_list = []
